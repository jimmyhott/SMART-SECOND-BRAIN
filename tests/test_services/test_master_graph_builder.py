...
try:
    ...
    logger.warning(
        "⚠️  Real components not available. Install langchain packages for integration tests."
    )

try:
    from langchain.schema import Document
except ImportError:  # pragma: no cover - fallback when langchain not installed
...
class TestMasterGraphBuilder:
    ...
    def mock_retriever(self):
        """Create a mock document retriever compatible with hybrid retrieval."""
        mock = Mock()

        class MockRetrievalResult:
            def __init__(self):
                self.document = Document(
                    page_content="This is a retrieved document for testing.",
        ...
        def mock_retrieve(query, k=8, filters=None, use_hybrid=True, min_score=0.1):
            return [retrieval_result]

        mock.retrieve.side_effect = mock_retrieve
        mock.retrieve_with_reranking.side_effect = (
            lambda query, k=8, rerank_top_k=12: [retrieval_result]
        )
        mock.get_relevant_documents.side_effect = lambda query: [retrieval_result.document]
        return mock
    ...
    @pytest.fixture
    def graph_builder(self, request, mock_llm, mock_embedding_model, mock_retriever, mock_vectorstore):
        ...
        # Replace default checkpointer with per-test SQLite database
        fd, checkpoint_path = tempfile.mkstemp(prefix="checkpoint_", suffix=".sqlite")
        os.close(fd)

        try:
            builder.checkpointer.conn.close()
        except Exception:
            pass

        checkpoint_conn = sqlite3.connect(checkpoint_path, check_same_thread=False)
        builder.checkpointer = SqliteSaver(checkpoint_conn)
    ...
    @pytest.fixture
    def real_graph_builder(self, request, temp_chromadb_dir):
        ...
        try:
            ...
            # Use isolated checkpointer per test to avoid sqlite corruption across runs
            if SqliteSaver is not None:
                checkpoint_path = os.path.join(temp_chromadb_dir, "checkpoints.sqlite")
                if os.path.exists(checkpoint_path):
                    os.remove(checkpoint_path)
                ...
                builder.checkpointer = SqliteSaver(sqlite_conn)
    ...
    def test_chunk_doc_node(self, graph_builder, sample_ingest_state):
        ...
        state = graph_builder.preprocess_node(sample_ingest_state)
    ...
    def test_chunk_doc_node_empty_document(self, graph_builder):
        ...
        state = graph_builder.preprocess_node(state)
        result_state = graph_builder.chunk_doc_node(state)
    ...
    def test_embed_node(self, graph_builder, sample_ingest_state):
        ...
        state = graph_builder.preprocess_node(sample_ingest_state)
        state = graph_builder.chunk_doc_node(state)
    ...
    def test_preprocess_auto_extracts_metadata(self, graph_builder):
        """Auto preprocessing should infer keywords and categories when enabled."""
        state = KnowledgeState(
            query_type="ingest",
            raw_document="Generative AI systems rely on transformers and attention mechanisms for contextual understanding.",
            categories=[],
            metadata={},
            auto_preprocess=True
        )

        result = graph_builder.preprocess_node(state)

        assert result.auto_generated_keywords
        assert result.auto_generated_categories
        assert result.metadata.get("auto_preprocess") is True
        assert "auto_generated_keywords" in result.metadata
        assert "auto_generated_categories" in result.metadata
        assert result.metadata.get("keywords")
        assert result.metadata.get("categories")
        assert result.categories  # categories should be populated

    def test_preprocess_merges_existing_metadata(self, graph_builder):
        """Auto preprocessing should respect existing metadata while appending inferred tags."""
        state = KnowledgeState(
            query_type="ingest",
            raw_document="Transformers enable contextual embeddings in modern AI pipelines.",
            categories=["existing"],
            metadata={"keywords": "custom", "categories": "existing"},
            auto_preprocess=True
        )

        result = graph_builder.preprocess_node(state)

        assert result.metadata.get("keywords") == "custom"

        assert result.categories[0] == "existing"
        assert len(set(result.categories)) == len(result.categories)
        assert any(cat != "existing" for cat in result.categories)

        assert result.auto_generated_keywords
        assert result.auto_generated_categories

    def test_preprocess_disabled_with_metadata_proceeds(self, graph_builder):
        """Providing metadata should allow ingestion even when auto preprocessing is disabled."""
        state = KnowledgeState(
            query_type="ingest",
            raw_document="Neural networks require labeled data for supervised learning.",
            categories=["ml"],
            metadata={"keywords": "neural, learning"},
        ...
        assert result.categories == ["ml"]

        chunked = graph_builder.chunk_doc_node(result)
        assert chunked.chunks

        embedded = graph_builder.embed_node(chunked)
    ...
    def test_long_paragraph_chunk_embed_store(self, real_graph_builder):
        ...
        try:
            ...
            assert result["status"] in {"stored", "validated", "validated_no_store"}
    ...
    def test_store_node(self, graph_builder, sample_ingest_state, mock_vectorstore):
        ...
        assert result_state.status in {"stored", "validated", "validated_no_store"}
        # Verify vectorstore was called
        assert mock_vectorstore.add_documents.called or mock_vectorstore.add_texts.called
    ...
    def test_retriever_node(self, graph_builder, sample_query_state, mock_retriever):
        ...
        assert mock_retriever.retrieve.called or mock_retriever.get_relevant_documents.called
...
"""
        ...
        with patch("builtins.open", mock_open(read_data=mock_prompt_content)) as _mock_file:
            with patch("agentic.workflows.master_graph_builder.ChatPromptTemplate") as mock_prompt_class:
                mock_prompt = Mock()
                mock_prompt.input_variables = ["conversation", "query", "context", "citations"]
                ...
                mock_prompt.bind.return_value = mock_prompt
    ...
    def test_real_llm_answer_generation(self, real_graph_builder, sample_query_state):
        ...
        logger.info(f"🤖 Real LLM Response: {result_state.generated_answer}")
    ...
    def test_real_document_ingestion_with_vectorstore(self, real_graph_builder, sample_documents):
        ...
        try:
            ...
            # LangGraph returns a dict, not a KnowledgeState object
            assert result["status"] in {"validated", "validated_no_store"}
            assert result["generated_answer"] is not None
            assert result["retrieved_docs"] is not None
    ...
    def test_real_workflow_with_text_splitter(self, real_graph_builder, sample_documents):
        ...
        try:
            ...
            assert len(result.get("chunks", [])) > 1  # Should be split into multiple chunks
            
            logger.info(f"✅ Document split into {len(result.get('chunks', []))} chunks")
            ...
            assert len(result.get("generated_answer", "")) > 50
