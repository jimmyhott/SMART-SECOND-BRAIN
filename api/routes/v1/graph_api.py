...
def create_graph_builder_with_collection(collection_name: str) -> MasterGraphBuilder:
    ...
    try:
        ...
        graph_builder = MasterGraphBuilder(
            ...
            vectorstore=vectorstore,                    # Vector database for storage
            chromadb_dir="./chroma_db",                 # ChromaDB storage directory
            collection_name=collection_name             # ChromaDB collection name
        )
...
class IngestRequest(BaseModel):
    ...
    auto_preprocess: bool = Field(False, description="Enable automated metadata extraction during ingestion")
...
class QueryRequest(BaseModel):
    ...
    metadata_filters: Optional[Dict[str, Any]] = Field(
        ...
        description="Metadata filters applied during retrieval (sources, categories, knowledge types)."
    )
    retrieval_options: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Retriever configuration overrides (k, use_hybrid, min_score, rerank_top_k)."
    )
...
async def ingest_document(
    ...
    try:
        ...
        state = KnowledgeState(
            ...
            auto_preprocess=request.auto_preprocess
        )
...
async def ingest_multiple_pdfs(
    ...
    auto_preprocess: bool = Form(False),
