...
class KnowledgeState(BaseModel):
    ...
    doc_id: Optional[str] = Field(
        None, description="Stable identifier for the ingested document."
    )
    """
    Stable document identifier used across chunk metadata, retrieval, and storage.
    Typically computed as a hash of the document content or a UUID supplied by the caller.
    """

    ingested_at: Optional[str] = Field(
        None, description="ISO timestamp recorded when the document was ingested."
    )
    """
    Timestamp associated with the ingestion event. Helpful for freshness filters,
    auditing, and lifecycle management of knowledge.
    """
    ...
    auto_preprocess: bool = Field(
        False,
        description=(
            "Enable automated metadata extraction prior to chunking when True. "
            "When False, the workflow expects caller-supplied metadata."
        )
    )

    auto_generated_keywords: Optional[List[str]] = Field(
        None,
        description="Keywords inferred during automated preprocessing."
    )

    auto_generated_categories: Optional[List[str]] = Field(
        None,
        description="Categories inferred during automated preprocessing."
    )

    chunk_metadata: Optional[List[Dict[str, Any]]] = Field(
        default_factory=list,
        description="Per-chunk metadata aligned with state.chunks order."
    )
    """
    Structural metadata describing each generated chunk. Entries may include:
        - chunk_id / chunk_index
        - doc_id and source
        - character offsets or page numbers
        - knowledge_type / categories / tags
    Used when writing to vector stores and when constructing citations.
    """
    ...
    metadata_filters: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Client-supplied filters applied during retrieval."
    )
    """
    Declarative filter specification passed to the retriever. Example keys include:
        - sources: ["manual", "pdf_ingest"]
        - categories: ["ai", "strategy"]
        - knowledge_types: ["reusable", "verified"]
        - date_range: {"gte": "2024-01-01", "lte": "2024-12-31"}
    """

    retrieval_options: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Additional retriever configuration such as k, rerank, min_score."
    )
    """
    Optional overrides controlling retrieval behaviour:
        - k: number of initial results to fetch
        - use_hybrid: enable hybrid (semantic + keyword) retrieval
        - min_score: drop results below similarity threshold
        - rerank_top_k: apply reranker to this many candidates
    """

    retrieval_log: Optional[List[Dict[str, Any]]] = Field(
        default_factory=list,
        description="Structured log capturing retrieval outcomes for observability."
    )
    """
    Captures per-query retrieval diagnostics such as selected chunk IDs, scores,
    applied filters, and reranking decisions. Facilitates monitoring and HITL review.
    """

    citation_map: Optional[List[Dict[str, Any]]] = Field(
        default_factory=list,
        description="Ordered list of snippets/citations used during answer generation."
    )
    """
    Provides metadata for each supporting snippet presented to the LLM, enabling
    answer templates to reference them (e.g., "[1]") and display provenance.
    """
