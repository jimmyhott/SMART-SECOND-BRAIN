...
if 'retrieval_options' not in st.session_state:
    st.session_state.retrieval_options = {}
if 'auto_preprocess' not in st.session_state:
    st.session_state.auto_preprocess = False
...
def ingest_multiple_pdfs(uploaded_files: List, source: str, categories: str = "", author: str = "", metadata: str = "", auto_preprocess: bool = False):
    ...
    try:
        ...
        data = {
            ...
            "auto_preprocess": str(bool(auto_preprocess)).lower()
        }
...
def process_text_ingestion(content, source, categories=None, author=None, knowledge_type="reusable"):
    ...
    try:
        ...
        data = {
            ...
            "auto_preprocess": st.session_state.get("auto_preprocess", False)
        }
...
def query_knowledge(query: str, thread_id: str = None):
    ...
    try:
        ...
        "retrieval_options": st.session_state.get("retrieval_options", {})
        }
        ...
        if "error" not in result:
            ...
            if isinstance(generated_answer, str):
                ...
                if normalized_answer.startswith("```"):
                    ...
                    # Handle optional language tag like ```json
                    if normalized_answer.lower().startswith("json"):
                        normalized_answer = normalized_answer[4:]
                    normalized_answer = normalized_answer.strip()
                    if normalized_answer.endswith("```"):
                        normalized_answer = normalized_answer[:-3]
                    generated_answer = normalized_answer.strip()

                if generated_answer.strip().startswith('{'):
                    try:
                        import json
...
def main():
    ...
    auto_preprocess_enabled = st.sidebar.checkbox(
        "Enable Auto Metadata Preprocessing",
        value=st.session_state.get("auto_preprocess", False),
        help="When enabled, inferred keywords and categories will be added if you leave them blank during ingestion."
    )
    with st.sidebar.expander("Retrieval Filters", expanded=False):
        filter_sources_raw = st.text_input(
            "Sources (comma-separated)",
            value="",
            help="Limit retrieval to specific sources"
        )
        filter_categories_raw = st.text_input(
            "Categories (comma-separated)",
            value="",
            help="Limit retrieval to specific categories"
        )
        knowledge_types = st.multiselect(
            "Knowledge Types",
            options=["conversational", "reusable", "verified"],
            default=[],
            help="Filter by knowledge type"
        )

    with st.sidebar.expander("Retriever Options", expanded=False):
        k_results = st.number_input(
            "Top K Results",
            min_value=1,
            max_value=20,
            value=8,
            help="Number of results to fetch before reranking"
        )
        min_score = st.slider(
            "Minimum Score",
            min_value=0.0,
            ...
            value=0.1,
            step=0.05,
            help="Filter out matches below this similarity score"
        )
        use_hybrid = st.checkbox(
            "Use Hybrid Retrieval",
            value=True,
            help="Combine semantic and keyword retrieval"
        )
        rerank_top_k = st.number_input(
            ...
            max_value=50,
            value=12,
            help="Number of candidates considered during reranking (0 disables)"
        )

    # Update session state with parsed filters and options
    filters = {}
    if filter_sources_raw.strip():
        filters["sources"] = [s.strip() for s in filter_sources_raw.split(",") if s.strip()]
    if filter_categories_raw.strip():
        filters["categories"] = [c.strip() for c in filter_categories_raw.split(",") if c.strip()]
    if knowledge_types:
        filters["knowledge_types"] = knowledge_types
    st.session_state.metadata_filters = filters
    ...
    st.session_state.auto_preprocess = bool(auto_preprocess_enabled)
                        ...
                        with st.expander("📚 View Citations", expanded=False):
                            for citation in citations:
                                ...
                                if metadata.get("original_filename"):
                                    st.markdown(f"• File: {metadata['original_filename']}")
                                if metadata.get("score"):
                                    st.markdown(f"• Score: {metadata['score']:.2f}")
                                if reason:
